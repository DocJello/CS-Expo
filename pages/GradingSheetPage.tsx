...
import { getGroupById, submitGrade } from '../services/mockApi';
...
const GradingSheetPage: React.FC = () => {
  ...
  const allPresenterScoresSet = BEST_PRESENTER_RUBRIC.every(item => presenterScores[item.id] !== undefined);
  const allThesisScoresSet = BEST_THESIS_RUBRIC.every(item => thesisScores[item.id] !== undefined);
