...
import { getGroups, getUsers, updateGroup } from '../services/api';
...
const MasterlistPage: React.FC = () => {
    ...
    const handlePanelChange = async (groupId: string, panelType: 'panel1Id' | 'panel2Id' | 'externalPanelId', panelistId: string) => {
        ...
        try {
            ...
            setGroups(prev => prev.map(g => g.id === groupId ? result : g));
                            ...
                            {groups.map(group => {
                                ...
                                <tr key={group.id}>
                                    ...
                                    <td className="px-6 py-4">
                                        <select value={group.externalPanelId || ''} onChange={e => handlePanelChange(group.id, 'externalPanelId', e.target.value)} className="w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600">
                                            <option value="">Assign...</option>
                                            {externalPanels.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                                        </select>
                                    </td>
                                    <td className="px-6 py-4">
                                        <select value={group.panel1Id || ''} onChange={e => handlePanelChange(group.id, 'panel1Id', e.target.value)} className="w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600">
                                            <option value="">Assign...</option>
                                            {internalPanels.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                                        </select>
                                    </td>
                                    <td className="px-6 py-4">
                                         <select value={group.panel2Id || ''} onChange={e => handlePanelChange(group.id, 'panel2Id', e.target.value)} className="w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600">
                                            <option value="">Assign...</option>
                                            {internalPanels.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                                        </select>
                                    </td>
