...
const initializeDatabase = async () => {
  ...
  try {
    ...
    // Ensure the admin user exists and has the correct password on every startup.
    // This uses an "UPSERT" (INSERT ON CONFLICT) to either create the admin
    // or update its password if it already exists. This is more robust
    // and guarantees the admin account is always accessible with the default password.
    console.log('Ensuring default admin user is correctly configured...');
    const salt = await bcrypt.genSalt(10);
    const password_hash = await bcrypt.hash('123', salt);
    
    await client.query(
      `INSERT INTO users (name, email, role, password_hash)
       VALUES ('User Admin', 'admin@example.com', 'Admin', $1)
